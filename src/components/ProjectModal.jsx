...
const ProjectModal = ({ onClose, ProjectInfo , ModalTheme}) => {
  ...
  return (
    <div className='fixed inset-0 bg-opacity-90 z-50 bg-black px-24 py-8 max-md:px-4 max-md:py-4 flex items-center justify-center'>
      <div className="bg-white rounded-xl shadow-lg relative flex flex-col h-full lg:flex-row max-md:h-auto md:w-full max-w-6xl">
        <button
          className='absolute right-4 top-4 max-md:right-2 max-md:top-2 z-50 rounded-md  hover:scale-105 transition-transform text-white bg-black'
          onClick={handleCloseModal}
        >
          <BiX size={25} />
        </button>

        <div className={`${ModalTheme} flex items-center justify-center lg:w-2/3 max-md:w-full p-8 lg:rounded-l-xl max-md:rounded-t-xl`}>
          <Carousel list={ProjectInfo.projectImages} />
        </div>

        <div className='lg:w-1/3 w-full px-8 py-6 max-md:px-4 max-md:py-4 flex flex-col justify-between'>
          ...
          <div>
            ...
            <div className='flex flex-wrap gap-2 overflow-y-auto h-24 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200'>
              {ProjectInfo.projectTechnologies.map((value, index) => (
                <div key={index} className='bg-slate-200 rounded-md text-sm h-10 w-28 flex justify-center items-center gap-x-2 transition-transform transform hover:scale-105'>
